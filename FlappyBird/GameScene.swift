...
//  FlappyBird
//
//  Created by mayuka on 2018/06/02.
...
class GameScene: SKScene,SKPhysicsContactDelegate{
    ...
    var itemSoreLabelNode:SKLabelNode!
    
    //アイテム取得音
    let get = SKAction.playSoundFileNamed("itemUp.mp3", waitForCompletion: true)
    //衝突時落下音
    let damage =  SKAction.playSoundFileNamed("damage1.mp3", waitForCompletion: true)
    
    // SKView上にシーンが表示されたときに呼ばれるメソッド
    override func didMove(to view: SKView) {
        
        //bgmを設定
        let url = Bundle.main.bundleURL.appendingPathComponent("bird_music.mp3")
        do{
    ...
    func setupItem(){
        ...
        // 画面外まで移動するアクションを作成
        let moveHeart = SKAction.moveBy(x: -movingDistance, y: 0, duration:3.0)
        ...
        let createheartAnimation = SKAction.run({
            ...
            //アイテム関連のノード乗せるノード
            let item = SKNode()
            item.position = CGPoint(x: self.frame.size.width + heartTexture.size().width / 2, y: 0.0)
            item.zPosition = -50.0 // 雲より手前、地面より奥
            
            // 画面のY軸の中央値
            let center_y = self.frame.size.height / 2
            // アイテムのY座標を上下ランダムにさせるときの最大値
            let random_y_range = self.frame.size.height * 0.4
            // アイテムのY軸の下限
            let heart_lowest_y = UInt32(center_y * 0.7)
            // 1〜random_y_rangeまでのランダムな整数を生成
            let random_y = arc4random_uniform( UInt32(random_y_range) )
            // Y軸の下限にランダムな値を足して、アイテムのY座標を決定
            let under_wall_y = CGFloat(random_y + heart_lowest_y)
            
            // スプライトを作成
            let heart = SKSpriteNode(texture: heartTexture)
            heart.position = CGPoint(x: 0.0, y: under_wall_y)
            heart.name = "heart"
            heart.xScale = 0.3
            heart.yScale = 0.4
            
            //物理演算を設定
            heart.physicsBody = SKPhysicsBody(circleOfRadius: heart.size.height / 2.0)
    ...
    func setupScoreLabel() {
        ...
        //アイテムスコア表示
        itemScore = 0
        itemSoreLabelNode = SKLabelNode()
        itemSoreLabelNode.fontColor = UIColor.black
        itemSoreLabelNode.position = CGPoint(x: 10, y: self.frame.size.height - 120)
        itemSoreLabelNode.zPosition = 100
        itemSoreLabelNode.horizontalAlignmentMode = SKLabelHorizontalAlignmentMode.left
        itemSoreLabelNode.text = "Item Score:\(itemScore)"
        self.addChild(itemSoreLabelNode)
    }
